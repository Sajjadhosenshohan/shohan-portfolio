...
export function SkillKey({ position, skill }: SkillKeyProps) {
  ...
  useFrame(() => {
    ...
    if (pressed) {
      meshRef.current.position.y = THREE.MathUtils.lerp(meshRef.current.position.y, position[1] - 0.1, 0.3)
    }
  })
  ...
  return (
    <group position={position}>
      ...
      <mesh
        ...
        onClick={() => {
          // Show skill info or trigger animation
      ...
      >
        ...
        <Html position={[0, 0.2, 0]} center distanceFactor={10} zIndexRange={[100, 0]}>
          <div className="flex items-center justify-center w-10 h-10 text-white" style={{ pointerEvents: "none" }}>
            {skill.icon}
          </div>
        </Html>
      </mesh>
